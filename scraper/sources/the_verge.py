...
def _is_article(url: str, title: str = "") -> bool:
    blocked_url = ("/video/", "/live-updates/", "/live/", "/liveblog/", "/short-form/")
...
def _fetch_feed(url: str) -> list[dict]:
    ...
    if entries:
        ...
        for entry in entries:
            ...
            teaser = html.unescape((entry.findtext(f"{{{ns}}}summary") or "").strip())
            pub_date = (entry.findtext(f"{{{ns}}}published") or entry.findtext(f"{{{ns}}}updated") or "").strip()
...
def _is_today_atom(pub_date: str, today: str) -> bool:
    # Atom dates are ISO 8601: 2026-03-16T14:45:00Z
    try:
...
def scrape(limit: int | None = None, delay: float = 0.75) -> list[dict]:
    ...
    all_articles = [
        ...
        if _is_today(a.get("publishedAt", ""), today) or _is_today_atom(a.get("publishedAt", ""), today)
    ]
