...
def _is_today(pub_date: str, today: str) -> bool:
    try:
        # DW uses ISO 8601 (dc:date), e.g. "2026-03-16T14:45:00Z"
...
def _is_article(url: str, title: str = "") -> bool:
    blocked_url = ("/video/", "/live-updates/", "/live/", "/liveblog/", "/av/")
...
def _fetch_feed(url: str) -> list[dict]:
    ...
    for item in root.findall("{http://purl.org/rss/1.0/}item"):
        loc = (item.findtext("{http://purl.org/rss/1.0/}link") or "").strip()
        title = (item.findtext("{http://purl.org/rss/1.0/}title") or "").strip()
        teaser = (item.findtext("{http://purl.org/rss/1.0/}description") or "").strip()
        pub_date = (item.findtext("{http://purl.org/dc/elements/1.1/}date") or "").strip()
